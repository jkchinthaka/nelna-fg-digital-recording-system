...
def _perm(codename: str, app_label: str = "accounts") -> Permission:
    ...
    if existing:
        return existing
    ct, _ = ContentType.objects.get_or_create(app_label=app_label, model=f"synthetic_{codename}")
...
@pytest.mark.django_db
def test_list_helpers_and_catalogue_buckets() -> None:
    from apps.access_control.governance_services import (
        list_proposed_business_categories,
        list_sod_open_questions,
    )
    from apps.access_control.permission_catalogue import (
        CapabilityBucket,
        entries_for_bucket,
        technical_permission_codenames,
    )

    sod = list_sod_open_questions()
    assert len(sod) == 6
    assert all(q["status"] == "PENDING" for q in sod)
    cats = list_proposed_business_categories()
    assert len(cats) >= 9
    assert all(c["approved"] == "No" for c in cats)
    assert entries_for_bucket(CapabilityBucket.RECORD)
    assert "scheduling.record_checklisttask" in technical_permission_codenames()


@pytest.mark.django_db
def test_set_role_permissions_rejects_unknown_ids() -> None:
    actor = make_user(employee_code="TST03C14")
    role = create_role(code="ROLE03C_BADP", name="Bad perms")
    with pytest.raises(ValidationError):
        set_role_permissions(actor=actor, role_id=role.id, permission_ids=[99999999])
