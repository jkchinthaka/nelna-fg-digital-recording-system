...
from apps.evidence.forms import EvidenceUploadForm, EvidenceRetireForm
...
def evidence_upload(request: HttpRequest, linked_kind: str, linked_object_id: uuid.UUID) -> HttpResponse:
...
def evidence_list_for_link(
    ...
    if linked_kind not in EvidenceLinkedKind.values:
        raise Http404("Unknown evidence link kind.")
    rows = list(list_evidence_for_link(_actor(request), linked_kind=linked_kind, linked_object_id=linked_object_id))
    return render(
        request,
        "evidence/list.html",
        {
...
@login_required
@require_GET
def evidence_download(request: HttpRequest, attachment_id: uuid.UUID) -> HttpResponse:
    _require_module(request)
    try:
