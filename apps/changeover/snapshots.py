...
def build_frozen_changeover_context(record: ChangeoverRecord) -> dict[str, Any]:
    return {
        ...
        "packaging_artwork_hook_id": (
            str(record.packaging_artwork_hook_id)
            if record.packaging_artwork_hook_id
            else None
        ),
        "previous_declaration_id": (
            str(record.previous_declaration_id)
            if record.previous_declaration_id
            else None
        ),
        "next_declaration_id": (
...
def build_frozen_line_clearance_context(record: LineClearanceRecord) -> dict[str, Any]:
    return {
        ...
        "checklist_submission_id": (
            str(record.checklist_submission_id)
            if record.checklist_submission_id
            else None
        ),
        "packaging_artwork_hook_id": (
            str(record.packaging_artwork_hook_id)
