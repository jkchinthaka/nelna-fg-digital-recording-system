...
| scheduling | Batch-triggered checklist task orchestration (07A); batch-source contract + integration port + recording permission foundation (07B); applicability (07C); effective-version task helper (07D) | ChecklistTask (`batch_reference`; no ProductionBatch master) | create/cancel services; `accept_batch_checklist_task_request` port; scoped selectors; orchestration UI; applicability preview; effective-version create helper | Must not store answers; no DRAFT/RETIRED tasks; no silent auto-latest; overlap/NO_ELIGIBLE must BLOCK; no invented batch ERP fields; no auto role mapping; manage ≠ record | **07A/07B/07C/07D** |
...
| 07 | **07A** ChecklistTask foundation; **07B** batch-source contract; **07C** applicability; **07D** effective-version policy; later recurrence/assignment as approved |
