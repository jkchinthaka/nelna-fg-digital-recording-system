...
| master_data | Configurable unseeded FG Product (05A); object-aware UI authz (05B); optional mapping fields + controlled CSV import (05C); other masters only when approved | FGProduct | Product create/update/activate/deactivate; controlled import; scoped selectors; management UI search/filter | Must not invent product catalogues; no silent/live ERP DB import; MASTER-001 unresolved | **05A/05B/05C technical** (catalogue pending) |
...
| recording | Operator draft recording (08A), immutable submissions (08B), controlled correction/resubmission (09B); server evaluation apply (06K) | ChecklistRecord, ChecklistResponse (+ evaluation_*), ChecklistSubmission, ChecklistSubmissionResponse (+ frozen evaluation_*), ChecklistCorrection | start/save/submit; start/resubmit correction; scoped selectors; draft/submitted/correction UI; server-authoritative item evaluation display | Must not mutate immutable snapshots/reviews; manage/review ≠ record; no QA/HOLD; evaluation ≠ disposition; no FG-QA-001 publish | **08A–09B** + **06K** |
