...
| scheduling | Batch-triggered checklist task orchestration (07A); batch-source contract + integration port + recording permission foundation (07B); applicability (07C); effective-version task helper (07D); recurring schedules (07E); batch-event adapter boundary (07F) | ChecklistTask (`batch_reference`; ExternalBatchMapping/Event; no ProductionBatch master) | create/cancel services; `accept_batch_checklist_task_request` / `accept_external_batch_event` ports; scoped selectors; orchestration UI; applicability preview; effective-version create helper; schedule generation | Must not store answers; no DRAFT/RETIRED tasks; no silent auto-latest; overlap/NO_ELIGIBLE must BLOCK; no invented batch ERP fields/endpoints/credentials; no live connector without APR-011; no auto role mapping; manage ≠ record | **07A–07F** |
...
| 07 | **07A** ChecklistTask foundation; **07B** batch-source contract; **07C** applicability; **07D** effective-version; **07E** recurring schedules; **07F** batch-event adapter (live contract required); later assignment as approved |
